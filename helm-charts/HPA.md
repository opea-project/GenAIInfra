...
However, Helm does not allow OPEA chart to overwrite Prometheus-adapter configMap, as
it belongs to another chart. Therefore manual step is needed to overwrite its current
custom metric rules.

Following will overwrite current Prometheus-adapter custom metric rules with OPEA Helm
generated ones, and restart Prometheus-adapter to apply the new rules:

```console
scripts/install-custom-metrics.sh monitoring chatqna
```

(Prometheus-adapter is assumed to be in `monitoring` namespace, and new rules to be
generated by a ChatQnA chart release named as `chatqna`.)

YAML backup of new and previous rules are saved to current directory.
...
And reranking service is used only after query context data has been uploaded. Before that,
you will not see any metrics!
