...
on kubernetes. It can leverage the microservices provided by [GenAIComps](https://github.com/opea-project/GenAIComps) and external services to compose GenAI pipelines. It also allows users to adjust the pipeline on the fly like switching to different Large language Models(LLM), add new functions into the chain(like adding guardrail),etc. GMC supports different types of steps in the pipeline, like sequential, parallel and conditional.

Please reference this [usage_guide](./usage_guide.md) for sample user cases.
