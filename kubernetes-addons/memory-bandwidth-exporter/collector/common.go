...
var (
	nodeName                  = kingpin.Flag("collector.node.name", "Give node name.").Default("").String()
	namespaceWhiteList        = kingpin.Flag("collector.container.namespaceWhiteList", "Filter out containers whose namespaces belong to the namespace whitelist, namespaces separated by commas, like \"xx,yy,zz\".").Default("").String()
	monTimes                  = kingpin.Flag("collector.container.monTimes", "Scan the pids of containers created before the exporter starts to prevent the loss of pids.").Default("10").Int()
	containerCollectorMetrics = kingpin.Flag("collector.container.metrics", "Enable container collector metrics").Default("all").String()
	classCollectorMetrics     = kingpin.Flag("collector.class.metrics", "Enable class collector metrics").Default("none").String()
	nodeCollectorMetrics      = kingpin.Flag("collector.node.metrics", "Enable node collector metrics").Default("none").String()
...
func processStats(oldStats RawStats, newStats RawStats) (ProcessedStats, error) {
	...
	if oldStats.CPUUtilization != nil && newStats.CPUUtilization != nil {
		...
		pstats.CPUUtilization = float64(newStats.CPUUtilization.CPU-oldStats.CPUUtilization.CPU) / float64(ncTime.Sub(ocTime).Microseconds())
	}
