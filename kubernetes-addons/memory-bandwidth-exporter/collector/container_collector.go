...
func (c *containerCollector) processContainerData(data map[string]info.ContainerInfo) error {
	for containerId, containerInfo := range data {
		...
		level.Info(c.logger).Log("msg", "ContainerInfoChan received", "operation", data[containerId].Operation, "pod name", data[containerId].PodName, "container id", containerId, "container name", data[containerId].ContainerName, "namespace", data[containerId].NameSpace)
...
func (c *containerCollector) updatePids(containerId string) {
	for i := 0; i < c.monTimes; i++ {
		c.logger.Log("debug", fmt.Sprintf("Scan for the %v time and update the pids of the container created before the exporter started.", i+1))
		if _, ok := c.containerInfos[containerId]; !ok {
			return
		}
		if err := writePidsToTasks(c.containerInfos[containerId].MonGroupPath, c.containerInfos[containerId].CgroupPath); err != nil {
			c.logger.Log("err", fmt.Sprintf("failed to update container %v stats: %v", containerId, err))
		}
		time.Sleep(jitter(c.interval, 0.1))
	}
}
...
func (c *containerCollector) housekeeping(containerId string) {
	...
	for range houseKeepingTimer.C() {
		...
		if os.IsNotExist(err) {
			c.logger.Log("info", fmt.Sprintf("container %v cgroup path %v does not exist, deleting cache", containerId, c.containerInfos[containerId].CgroupPath))
			delete(c.statsCache, containerId)
			delete(c.containerInfos, containerId)
			return
		}
		...
		houseKeepingTimer.Reset(jitter(c.interval, 0.1))
	}
}
...
func (c *containerCollector) updateStats(containerId string) error {
	...
	if isNeedCollectMbLLc(c.metrics) {
		newStats.SocketNum, newStats.MemoryBandwidth, newStats.Cache, err = getIntelRDTStatsFrom(c.containerInfos[containerId].MonGroupPath)
		if err != nil {
			return err
		}
	}
