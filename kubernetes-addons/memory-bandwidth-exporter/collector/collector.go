...
func registerCollector(collector string, isDefaultEnabled bool, factory func(logger log.Logger, interval time.Duration) (Collector, error)) {
...
func ParseCollectorMetrics() bool {
	...
	for collector := range collectorState {
		if collector == containerCollectorSubsystem {
			...
			if *containerCollectorMetrics == "none" {
			...
			if *containerCollectorMetrics == "all" || strings.Contains(*containerCollectorMetrics, "mb") || strings.Contains(*containerCollectorMetrics, "llc") {
