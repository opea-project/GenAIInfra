...
func main() {
	...
	if user, err := user.Current(); err == nil && user.Uid == "0" {
		level.Warn(logger).Log("msg", "Memory bandwidth exporter is running as root user. This exporter is designed to run as unprivileged user, root is not required.")
	}
	...
	go nriPlugin.Run(isNeedMakeMonitorGroup)
